...
function geolocationThatReturns(
    latitude: number,
    longitude: number,
): Geolocation {
    return {
...
describe("getUserLocation", () => {
    it("returns coordinates supplied by browser geolocation", async () => {
        await expect(
            getUserLocation(geolocationThatReturns(43.5, -80.5)),
        ).resolves.toEqual({
            latitude: 43.5,
            longitude: -80.5,
        });
    ...
    it("rejects when browser geolocation is unavailable", async () => {
        await expect(getUserLocation(undefined)).rejects.toThrow(
...
describe("getStartingLocation", () => {
    it("uses browser coordinates when permission is granted", async () => {
        await expect(
            getStartingLocation(geolocationThatReturns(43.5, -80.5)),
        ).resolves.toEqual({
            latitude: 43.5,
            longitude: -80.5,
        });
    ...
    it("uses the Waterloo default when geolocation fails", async () => {
        ...
        await expect(
            getStartingLocation(geolocationThatFails()),
        ).resolves.toEqual({
            ...UW_DEFAULT_LOCATION,
            usedDefault: true,
        });
