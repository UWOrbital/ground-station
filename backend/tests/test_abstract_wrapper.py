...
async def test_create(wrapper,obj1,obj2):
    result=await wrapper.create(obj1)
    ...
    result=await wrapper.create(obj2)
...
async def test_get_all(wrapper,obj1,obj2):
    await wrapper.create(obj1)
    await wrapper.create(obj2)
    arr=await wrapper.get_all()
...
async def test_get_by_id(wrapper,obj1):
    get=await wrapper.create(obj1)
    obj=await wrapper.get_by_id(get.id)
    assert obj.id==get.id
    random_id=uuid4()
    err=None
    try:
        obj=await wrapper.get_by_id(random_id)
...
async def test_delete_by_id(wrapper,obj1):
    delete=await wrapper.create(obj1)
    obj=await wrapper.delete_by_id(delete.id)
    assert obj.id==delete.id
    assert not await wrapper.get_all()
    random_id=uuid4()
    err=None
    try:
        obj=await wrapper.delete_by_id(random_id)
    except ValueError as e:
        err=type(e)
    finally:
        assert err==ValueError

async def test_update(wrapper,obj1):
    obj=await wrapper.create(obj1)
    assert obj.email==obj1["email"]
    new_info=obj1.copy()
    new_info["email"]="new_email@gmail.com"
    new_obj=await wrapper.update(obj.id,new_info)
