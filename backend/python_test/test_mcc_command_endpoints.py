...
def clear_pipeline_queue():
    """Clear the pipeline queue and disable lockout for each test"""
    from api.v1.mcc.endpoints.commands import COMMANDS_PIPELINE
...
def test_get_command_queue_empty(client):
    """Test getting an empty command queue"""
    response = client.get("/api/v1/mcc/commands/get_command_queue")
    assert response.status_code == 200
    assert response.json()["data"] == []


def test_queue_command_success(client):
    """Test successfully queuing a command (CMD_DOWNLINK_LOGS_NEXT_PASS = 3)"""
    payload = {
        "params": {
            "log_level": 1,
            "time_of_execution": 0
    ...
    response = client.post("/api/v1/mcc/commands/queue_command", json=payload)

    assert response.status_code == 200
    data = response.json()["data"]
    assert len(data) == 1
    assert data[0]["type_"] == 3

    queue_response = client.get("/api/v1/mcc/commands/get_command_queue")
    assert queue_response.status_code == 200
    queue_data = queue_response.json()["data"]
    assert len(queue_data) == 1
    assert queue_data[0]["type_"] == 3


def test_enable_queue_lockout(client):
    """Test enabling pipeline lockout"""
    response = client.post("/api/v1/mcc/commands/enable_queue_lockout")
    assert response.status_code == 200
    assert response.json()["message"] == "Lockout enabled, command insertion into queue will no longer be possible until lockout is disabled"
    ...
    client.post("/api/v1/mcc/commands/queue_command", json=payload)

    queue_response = client.get("/api/v1/mcc/commands/get_command_queue")
    assert queue_response.status_code == 200
    assert len(queue_response.json()["data"]) == 0


def test_disable_queue_lockout(client):
    """Test disabling pipeline lockout"""
    client.post("/api/v1/mcc/commands/enable_queue_lockout")

    response = client.post("/api/v1/mcc/commands/disable_queue_lockout")
    assert response.status_code == 200
    assert response.json()["message"] == "Lockout disabled, command insertion into queue now possible"
    ...
    queue_response = client.get("/api/v1/mcc/commands/get_command_queue")
    assert queue_response.status_code == 200
    assert len(queue_response.json()["data"]) == 1
